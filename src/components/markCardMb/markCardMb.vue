<template>
  <div class="markCard">
    <el-row class="mc_row">
      <el-col v-for="(item, index) in marketList" :key="index" :class="nftType===1?'mc_col2':'mc_col'">
        <div @click="handleClickItem(item)">
          <img class="mc_hot" :src="require(`../../assets/newPhones/nLaboratory/icon_coin_${item.headType}.png`)" alt="">
          <div class="mc_c_img"><img alt="logo" :src="item.picurl"></div>
          <div class="mc_c_content">
            <div class="mc_c_left">
              <div class="mc_c_title">{{ item.name }}</div>
              <div class="mc_c_price">
                <img src="../../assets/newPhones/universal/icon_usdt_coin.svg" alt="">
                <span>{{ item.sellprice  | noRoundingDecimals(2) }}</span>
              </div>
              <div class="mc_c_time">
                <img src="../../assets/newPhones/nMarket/icon_m_sz.svg" alt="logo">
                <span>{{ countdownTimeList[index] }}</span>
              </div>
            </div>
            <div class="mc_c_right">
              <div class="mc_c_go"><img src="../../assets/newPhones/nMarket/icon_m_buy.svg" alt=""></div>
            </div>
          </div>
        </div>
      </el-col>
    </el-row>
  </div>
</template>

<script>

import {mapGetters} from 'vuex';
import {timeFromNow} from '@/utils/computing'

export default {
  name: "markCard",
  props: ["marketList", "nftType"],
  data() {
    return {
      isEnglish: false,
      timingTimer: null, // 定时器
      countdownTimeList: [],// 倒计时数据数组
    };
  },
  created() {
    this.isEnglish = window.localStorage.getItem("lang") !== "zh";
    if (this.marketList.length > 0) {
      const that = this;
      this.marketList.forEach((item, index, array) => {
        const timeString = timeFromNow(true, item.updateTime);
        that.countdownTimeList.splice(index, 1, timeString);
      });
      this.startOpenTimerCountdown();
    }
    console.log('creat markCard card');
  },
  mounted() {

  },
  destroyed() {
    if (this.timingTimer) {
      clearInterval(this.timingTimer);
      this.timingTimer = null;
      console.log('退出页面关闭定时器');
    }
  },
  watch: {
    '$i18n.locale'(nval, oval) {
      if (nval === 'zh') {
        this.isEnglish = false;
      } else {
        this.isEnglish = true;
      }
      console.log('监听到语言的切换');
    },
    marketList(val, oval) {
      if (val.length > 0) {
        const that = this;
        val.forEach((item, index, array) => {
          const timeString = timeFromNow(true, item.updateTime);
          that.countdownTimeList.splice(index, 1, timeString);
        });
        this.startOpenTimerCountdown();
      }
      console.log('数据发生改变');
    }
  },
  // 计算属性
  computed: {
    ...mapGetters(['isConnectWallet']),
  },
  // 方法集合
  methods: {
    handleClickItem(item) {
      console.log('handleClickItem =', item)
      this.$emit("listenClickItem", item);
    },
    handleViewOnlyMethod(item) {
      this.$emit("handleViewOnlyMethod", item);
    },
    // 开启定时器
    startOpenTimerCountdown() {
      if (this.timingTimer) {
        clearInterval(this.timingTimer);
        this.timingTimer = null;
        console.log('关闭定时器，重新开启');
      }
      // 开启定时器
      const that = this;
      this.timingTimer = setInterval(() => {
        that.marketList.forEach((item, index, array) => {
          const timeString = timeFromNow(true, item.updateTime);
          that.countdownTimeList.splice(index, 1, timeString);
        });
      }, 60000);
    },
  },
};
</script>

<style lang="scss" scoped>
.markCard {
  @include size(100%, 100%);
  /deep/ .el-row::before {
    content: none !important;
  }
  /deep/ .el-row::after {
    content: none !important;
  }
  .mc_row {
    display: flex;
    flex-wrap: wrap;
    width: vw(690);
    margin: 0 auto;
    .mc_col {
      position: relative;
      width: vw(690);
      height: auto;
      margin: 0 auto vw(20);
      border-radius: vw(20);
      background-color: #2c2f53;
      overflow: hidden;
      .mc_hot {
        position: absolute;
        top: vw(10);
        left: 0;
        width: vw(60);
        height: vw(60);
      }
      .mc_c_img {
        width: vw(690);
        height: vw(690);
        box-sizing: border-box;
        overflow: hidden;
        img {
          width: 100%;
          height: 100%;
        }
      }
      .mc_c_content {
        display: flex;
        justify-content: space-between;
        padding: vw(20);
        .mc_c_left {
          .mc_c_title {
            font-family: PingFangSC, sans-serif;
            font-size: vw(26);
            font-weight: 500;
            font-stretch: normal;
            font-style: normal;
            line-height: normal;
            letter-spacing: normal;
            text-align: left;
            color: #fff;
            overflow: hidden;
            word-break: break-all;
            display: -webkit-box;
            -webkit-line-clamp: 2;
            -webkit-box-orient: vertical;
            text-overflow: ellipsis;
          }
          .mc_c_price {
            display: flex;
            align-items: center;
            margin-top: vw(14);
            img {
              width: vw(50);
              height: vw(50);
            }
            span {
              margin-left: vw(20);
              font-family: PingFangSC-Semibold, sans-serif;
              font-size: vw(32);
              font-weight: 600;
              font-stretch: normal;
              font-style: normal;
              line-height: normal;
              letter-spacing: normal;
              text-align: left;
              color: #ff953f;
            }
          }
          .mc_c_time {
            margin-top: vw(14);
            display: flex;
            align-items: center;
            img {
              width: vw(24);
              height: vw(24);
            }
            span {
              margin-left: vw(10);
              font-family: PingFangSC-Regular, sans-serif;
              font-size: vw(20);
              font-weight: 300;
              font-stretch: normal;
              font-style: normal;
              line-height: normal;
              letter-spacing: normal;
              text-align: left;
              color: #ff953f;
            }
          }
        }
        .mc_c_right {
          display: flex;
          align-items: flex-end;
          .mc_c_go {
            margin-bottom: vw(20);
            width: vw(60);
            height: vw(60);
            img {
              width: 100%;
              height: 100%;
            }
          }
        }
      }
    }
    .mc_col2 {
      position: relative;
      width: vw(335);
      height: vw(493);
      margin-left: vw(20);
      margin-bottom: vw(20);
      border-radius: vw(20);
      background-color: #2c2f53;
      overflow: hidden;
      &:nth-child(2n + 1) {
        margin-left: 0;
      }
      .mc_hot {
        position: absolute;
        top: vw(10);
        left: 0;
        width: vw(60);
        height: vw(60);
      }
      .mc_c_img {
        width: vw(335);
        height: vw(335);
        box-sizing: border-box;
        overflow: hidden;
        img {
          width: 100%;
          height: 100%;
        }
      }
      .mc_c_content {
        display: flex;
        justify-content: space-between;
        padding: vw(12);
        .mc_c_left {
          .mc_c_title {
            font-family: PingFangSC-Regular, sans-serif;
            font-size: vw(24);
            font-weight: 500;
            font-stretch: normal;
            font-style: normal;
            line-height: normal;
            letter-spacing: normal;
            text-align: left;
            color: #fff;
            overflow: hidden;
            white-space: nowrap;
            word-break: break-all;
            text-overflow: ellipsis;
          }
          .mc_c_price {
            display: flex;
            align-items: center;
            margin-top: vw(15);
            img {
              width: vw(34);
              height: vw(34);
            }
            span {
              margin-left: vw(10);
              font-family: PingFangSC-Semibold, sans-serif;
              font-size: vw(28);
              font-weight: 600;
              font-stretch: normal;
              font-style: normal;
              line-height: normal;
              letter-spacing: normal;
              text-align: left;
              color: #ff953f;
            }
          }
          .mc_c_time {
            margin-top: vw(19);
            display: flex;
            align-items: center;
            img {
              width: vw(20);
              height: vw(20);
            }
            span {
              margin-left: vw(10);
              font-family: PingFangSC-Regular, sans-serif;
              font-size: vw(20);
              font-weight: 300;
              font-stretch: normal;
              font-style: normal;
              line-height: normal;
              letter-spacing: normal;
              text-align: left;
              color: #ff953f;
            }
          }
        }
        .mc_c_right {
          display: flex;
          align-items: flex-end;
          .mc_c_go {
            margin-bottom: vw(20);
            width: vw(44);
            height: vw(44);
            img {
              width: 100%;
              height: 100%;
            }
          }
        }
      }
    }
  }
}
</style>
