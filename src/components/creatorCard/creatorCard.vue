  ...
  <div class="markCard">
    <el-row class="mc_row">
      <el-col class="mc_col" v-for="(item, index) in marketList" :key="index">
        <div @click="handleClickCollectionNFT(item)">
          <img class="mc_hot" :src="require(`../../assets/newPhones/nLaboratory/icon_coin_${item.headType}.png`)" alt="">
          <div class="mc_c_img"><img :src="item.picurl" alt="logo"></div>
          <div class="mc_c_top">
...
export default {
  ...
  methods: {
    ...
    handleClickCollectionNFT(item) {
      ...
      // 创造者详情列表是商店类/(普通商品/艺术类NFT)公用的页面
      if (this.typeName === 'shop') {
        // item = ID  item1 = 是否是商店商品
        const routeUrl = this.$router.resolve({path: '/nftdetial',query:{item: item.id,item1:true}});
